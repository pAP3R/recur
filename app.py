        ...
        if cash['currency'] == 'USD':
            if float(cash['balance']) >= float(quantity):
                ...
                if 'message' in res:
                    ...
                    conn.execute('INSERT INTO order_history (created, side, asset, quantity, total, frequency, exchange, type, order_details) VALUES (?,?,?,?,?,?,?,?,?,?)', (time.time(), side, asset, quantity, filled_size, order['frequency'], order['exchange'], order['type'], str(order_details[0])))
                    conn.commit()
                    conn.close()
        ...
        if cash["currency"] == "USD":
            ...
            if float(cash["balance"]) >= float(quantity):
                ...
                if "message" in res:
                    ...
                    conn.execute('INSERT INTO order_history (created, side, asset, quantity, total, frequency, exchange, type, order_details) VALUES (?,?,?,?,?,?,?,?,?)', (time.time(), "Buy", asset, quantity, filled_size, frequency, "Coinbase", "Market", str(order_details[0])))
                    conn.commit()
                    conn.close()
